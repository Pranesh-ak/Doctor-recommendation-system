...
def appointment(request,pid,did):
    if request.method=="POST":
        ...
        apps=Appointments.objects.filter(docid=did,appdate=date,apptime=time)
        if apps:
            ...
            query=Appointments(docid=did,docname=dname,pid=pid,pname=pname,appdate=date,apptime=time,desc=desc)
            query.save()
